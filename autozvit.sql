...
UPDATE `out`
    SET `N2` = (SELECT IFNULL(SUM(`N7`), 0) FROM `Z2MPZ`
        ...
        `N4` = (SELECT IFNULL(SUM(`N6`), 0) FROM `Z43MPS`
    ...
    SET `N2` = 
          (SELECT IFNULL(SUM(`N2`), 0) FROM `WM_1`
        	WHERE `WM_1`.`ID_KEY` = `out`.`ID_KEY`
                AND `KOD` IN (41010600, 41020300, 41035600, 41035200, 4103500, 41010900))
...
FROM
    ...
    LEFT JOIN
    (SELECT `ID_KEY`, SUM(`N1`) AS `n1_1D`, SUM(`N3`) AS `n3_1D` FROM `out` WHERE `KOD` = "1D" GROUP BY `ID_KEY`) USING(`ID_KEY`)
    LEFT JOIN 
    (SELECT `ID_KEY`, SUM(`N1`) AS `n1_208100`, SUM(`N3`) AS `n3_208100` FROM `out` WHERE `KOD` = 208100 GROUP BY `ID_KEY`) USING(`ID_KEY`)
    LEFT JOIN 
    (SELECT `ID_KEY`, SUM(`N1`) AS `n1_208400`, SUM(`N3`) AS `n3_208400` FROM `out` WHERE `KOD` = 208400 GROUP BY `ID_KEY`) USING(`ID_KEY`);

-- Сам запрос
UPDATE `out`
    SET `N1` =    (SELECT `n1_208400` - `n1_208100` - `n1_1D` FROM `out_view6` WHERE `ID_KEY` = `out`.`ID_KEY`),
        `N3` =    (SELECT `n3_208400` - `n3_208100` - `n3_1D` FROM `out_view6` WHERE `ID_KEY` = `out`.`ID_KEY`)
    WHERE `KOD` = 208200;
